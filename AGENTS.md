...
Small Python research workspace for figure-eight three-body simulations.

- `codexpy.py`: primary maintained simulation tool with SciPy integration, Lyapunov analysis, GPU ensemble diagnostics, plots, and reports.
...
There is no `tests/` directory or package layout. Add reusable code inside `codexpy.py` unless the project grows enough to justify modules.
...
```powershell
.\venv\Scripts\python.exe .\codexpy.py --quick --no-plots --ensemble-size 0
```
Fast CPU smoke test for solver, energy diagnostics, and Lyapunov calculation.

```powershell
.\venv\Scripts\python.exe .\codexpy.py --quick --gpu --ensemble-size 512 --ensemble-steps 300
```
Quick GPU validation using CuPy ensemble diagnostics.

```powershell
.\venv\Scripts\python.exe .\codexpy.py --quick --advanced --ic-mode random_chaotic --seed 42 --gpu --ensemble-size 256 --ensemble-steps 120
```
Fast advanced run with QR spectrum, Poincare, FFT, recurrence, FTLE, basin, and sensitivity diagnostics.

```powershell
.\venv\Scripts\python.exe .\codexpy.py --gpu
```
...
Avoid cleanup-only edits to `IDK.py`; treat `codexpy.py` as maintained.
...
If adding tests, create `tests/test_codexpy.py` and use `pytest` with `--quick`-style durations.
